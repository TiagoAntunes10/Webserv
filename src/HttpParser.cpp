...
//Checks Methods (GET, POST, DELETE) and checks bad requests and update the response status
...
unsigned short	HttpParser::parseHeader(const std::string& request, http_Data& httpReq) {
	responseSTAT = OK;
	
	//Check if request is valid
	if (request.empty() || (request.find_first_not_of("\t\r\n") == std::string::npos)) {
...
contains at most one '?', and has a valid number of '=' and '&' characters when 
a '?' is present (must match the number of '&' characters plus one). */
bool	HttpParser::isValidUrl(const std::string& url) {
	...
	if (std::count(url.begin(), url.end(), '?') > 1)
		return false;
	if ((url.find('?') != std::string::npos) 
		&& (std::count(url.begin(), url.end(), '=') != std::count(url.begin(), url.end(), '&') + 1))
		return false;
...
//trim whitespaces ( space \t \r \n) from begin and ened of a string. Return a trimmed result
...
std::ostream& operator<<(std::ostream& out, const http_Data& req) {
	...
	for (std::multimap<std::string, std::string>::const_iterator it = req.header.begin(); it != req.header.end(); it++)
		out << "\t" << it->first << ": " << it->second << std::endl;
	
	out << "Query parameters: " << std::endl;
	for (std::multimap<std::string, std::string>::const_iterator it = req.queryParams.begin(); it != req.queryParams.end(); it++)
		out << "\t" << it->first << ": " << it->second << std::endl;
